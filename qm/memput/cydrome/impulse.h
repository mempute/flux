...
public:
	...
	bool predwithloss, finalSync, runPredict;
	sytet tadapt, trainstep, init_trainstep;
	Isleloop *islelist;
	Dynagen *dgenlist;
	hmutex mutimp;
	lsemx semimp;
...
public:
	...
	bool fintrain, sockIndice[2], tadaptisle, alignoper, finalisle, fowardit, neverTrained;
	Cydrome *cydisle;
...
public:
	...
	void buildislet(Flux *in, intt &bound_sz)
	{
		...
		intt sz_slide;
		if(d2) {
			sz_slide = std::sqrt(isle_in->fshape[1] * isle_in->fshape[2] * isletrc->rkernal2d * isletrc->rkernal2d);
			if(sz_slide > isletrc->szslide2d) sz_slide = isletrc->szslide2d;
			else if(sz_slide < 4) {//프레임 사이즈를 1/8로 줄였을때 커널 사이즈가 (4*4)보다 작은 단계가 
				if(bound_sz == 1) bound_sz = -1;//되면 bound_sz가 1로 주어지면 마지막 압축되게 설정
				sz_slide = 4;//최소 커널 사이즈 설정.
			}
			/*else {
				if(bound_sz > 1) {//bound_sz로 마지막 압축을 체크하는 경우이면, bound_sz이 1이면 이조건 무시됨.
					//2d는 최종 압축이 아니면 정방형을 유지해야하는데(꼭 해야하는 것은 아님) bound_sz에 의해 최종
					//압축여부를 알수있는 것은 압축을 해봐야 알수있고 최종압축이어서 8개당 1개 압축코드를 생성하는것은
					//압축이전에 ㄱ)설정에서 되야하므로 bound_sz를 입력 사이즈로 하여 여기서 압축이전에 체크한다.
					if(isle_in->fshape[1] * isle_in->fshape[2] < bound_sz) {//입력 사이즈가 일정
						bound_sz = -1;//마지막 압축되게 하고 마지막이므로 정방형을 유지할 필요가  
						isletrc->lowbound = 0.125;//ㄱ.없으므로 8개당 1개 압축코드를 생성하게 한다.
						if(isletrc->printBuilding) printf("encord isle: less than bound sz so do not need square compress code\n");
					}
					if(sz_slide < 4) sz_slide = 4;//최소 커널 사이즈 설정.
				} else if(sz_slide < 4) {//프레임 상이즈를 1/8로 줄였을때 커널 사이즈가 (4*4)보다 작은 단계가 되면 
					bound_sz = -1;	//마지막 압축되게 하고 마지막이므로 정방형을 유지할 필요가 없으므로 
					isletrc->lowbound = 0.125;//8개당 1개 압축코드를 생성하게 한다.
					sz_slide = 4;//최소 커널 사이즈 설정.
					if(isletrc->printBuilding) printf("encord isle: slide size less than 4 * 4 so do not need square compress code\n");
				}
			}*/
		} else sz_slide = isletrc->szslide1d; 
		if(isletrc->samplingInterv) {//입력 데이타의 sampling이 조밀하게 되는 경우 그와 반비례 관계로
			//슬라이드 사이즈를 기준으로 인터벌을 띄엄띄엄 scooup한다.(이미 입력이 조밀하게 스트라이드됐으면 여기서 또 조밀할 필요업으니)
			isletrc->slTrainInterv = isletrc->slPredInterv = sz_slide / isletrc->samplingInterv;
		}
		...
		if(isletrc->printBuilding) {
			printf("encord isle: scoop out slide size %d ^ %d ", sz_slide, (intt)d2 + 1); scoopout->shape();
		}
		...
		if(d2) {
			...
			//bound_sz로 마지막 압축을 체크하는 경우이고 사이즈 이하이면 설정, bound_sz이 1이면 이조건 
			if(bound_sz > 1 && out_shape[1] * out_shape[2] < bound_sz) bound_sz = -1;//무시되고 위에서 체크
		} else {
		...
		if(isletrc->printBuilding) {
			if(d2) printf("encord isle: zcode sz: %d grid shape[%d, %d, %d, %d]\n",
				zcode_len * zcode_len, out_shape[0], out_shape[1], out_shape[2], out_shape[3]);
			else printf("encord isle: zcode sz: %d grid shape[%d, %d, %d]\n",
				zcode_len, out_shape[0], out_shape[1], out_shape[2]);
		}
	...
	Encodeisle(Impulse *imp, Dynagen *dgen, Flux *in_gate, intt &bound_sz, bool pair, sytet head, bytet *name, floatt lr = -1) : Isleloop(imp, dgen, head, name)
	{
		...
		if(pair) {
			if(isleimp->impTcr->nbatch2pseq) {//이옵션은 2d이미지를 여러개 배치를 시퀀스로 묶어 듀얼인코딩에서
				throwFault(-1, (bytex *)"not match opt\n");//사용하기위함이므로 이를 디코더의 입력으로 주입하면
			}												//배치가 틀려지므로 에러
...
public:
	...
	void buildislet(Flux *in, Flux *tar)
	{
		...
		convert_2d_to_1d(in, dims);
		if(isleimp->impTcr->nbatch2pseq) {//배치를 시퀀스로 전환
			dims[1] *= isleimp->impTcr->nbatch2pseq;//주어진 배치 갯수만큼 시퀀스로 전환
			dims[0] = (dims[0] + isleimp->impTcr->nbatch2pseq -1) / isleimp->impTcr->nbatch2pseq;
			if(isletrc->printBuilding) {
				printf("coax isle: conversion batch to seq n-shfit[%d] * seq-dim[%d] [%d, %d, %d]\n", 
					isleimp->impTcr->nbatch2pseq, dims[1]/isleimp->impTcr->nbatch2pseq, dims[0], dims[1], dims[2]);
			}
		}
		islein[0] = flux(isletrc, 3, dims, in->qType, variable);
		...
		if(isleimp->impTcr->bygate && szembede < 0) {//바이게이트가 설정됐고 classification에서
			...
			if(isleimp->impTcr->stracode) {//dncs.stracode이면 다이나젠에서 하위 인코딩하고 
				szembede = 0;//압축인코딩되었으므로 임베딩은 필요없고 아래 제너릭에서 한번더 중간 
			}				//인코딩후 최종 듀얼인코더에 입력하게 하도록 szembede를 0로 설정한다.
...
public:
	...
	Cell *coupnet(void)
	{
		return coupleisle->islenet;//제너릭 망 리턴
	}
		...
		intt embedim, sytet af = ACTF_TANH, floatt lr = -1, const bytet *name = nullx)
	{
		...
		if(t_dynagen == T_DG_TRANSLATE || t_dynagen == T_DG_CALSS || t_dynagen == T_DG_ENCODE) {
			...
			if(t_dynagen == T_DG_TRANSLATE) {
				...
				//Coaxisle의 입력이 인코더에서 인코딩된 압축코드이므로 임베딩 필요없고 
				//classification에서 호출 표시로 -1 설정. 듀얼인코더 설정일때 의미있는 것이고 
