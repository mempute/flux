...
def print_trainable_variables():
    print('kk')#display_var_info(tf.trainable_variables())
...
class BaseModelMixin:
    ...
    def save_checkpoint(self, step=None):
        print(" [*] Saving checkpoints...", "green")
        ckpt_file = os.path.join(self.checkpoint_dir + '\\' + self.model_name, self.model_name)
        self.saver.save(self.sess, ckpt_file, global_step=step)

    def load_checkpoint(self):
        print(" [*] Loading checkpoints...", "green")
        ckpt_path = tf.train.latest_checkpoint(self.checkpoint_dir + '\\' + self.model_name)
        ...
        if ckpt_path:
            ...
            print(" [*] Load SUCCESS: %s" % ckpt_path, "green")
            return True
        else:
            print(" [!] Load FAILED: %s" % self.checkpoint_dir, "red")
            return False
